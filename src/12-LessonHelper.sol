...
//         (bool successTwo, bytes memory ownerBytes) = exploitContract.call(abi.encodeWithSignature("getOwner()"));
    ...
    function getNumberr() public pure returns (bool, bytes memory) {
        uint128 res = 13;
        return (true, abi.encodePacked(res));
    }

    function getOwnerr() public view returns (bool, bytes memory) {
        return (true, abi.encodePacked(address(msg.sender)));
    }
